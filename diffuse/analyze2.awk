...
END {
    ...
    #print "filename                                  rel_energy  iter  link   crt_frce   rms_force    max_force     rms_disp    max_disp    forming    breaking    done   freqs"
    for (i=1; i <= fileCount; i++)
        {
            ...
            formingBond = distance(i,19,34)
            breakingBond1 = distance(i,34,53)
            breakingBond2 = distance(i,34,43)
            breakingBond3 = distance(i,34,52)
            if ( breakingBond1 <= breakingBond2 && breakingBond1 <= breakingBond3 )
                sulfonateOxygen = 53
            else if ( breakingBond2 <= breakingBond1 && breakingBond2 <= breakingBond3 )
                sulfonateOxygen = 43
            else
                sulfonateOxygen = 52
            
            breakingBond = breakingBond1 < breakingBond2 ? breakingBond1 : breakingBond2
            angle1 = angle(i,19,34,53)
            lithiumAlkoxide = distance(i,19,83)
            lithiumSulfonate = distance(i,sulfonateOxygen,83)
            carbonLithiumSulfonate = angle(i,34,83,sulfonateOxygen)
            dihedral1 = dihedral(i,35,42,34,19)
            isFinished=finished[i] == 0 ? "NO" : finished[i]
            imaginariesString = didFrequencies[i] ? "--" : ""
            if (i in imaginaries)
                imaginariesString = -1*imaginaries[i]
            lastLinkString = lastLink[i]
            #RMSdelta = calculateRMS(i,i-1)
            #print RMSdelta
            #printf "%35s %16.4f  %3d  %5s  %9s    %9s    %9s   %9s   %9s   %8.3f   %8.3f      %3s   %12s\n", filename, relEnergy, thisIterations, lastLinkString, cartForce, RMSforce, maxForce, RMSdisplacement, maxDisplacement, formingBond, breakingBond, isFinished, imaginariesString
            
            split(filename,fields,"-") # this is the level of theory
            
            printf "%s,%.3f,%.3f,%.3f,%s,%.3f,%.3f,%.3f,%.3f\n", fields[7], formingBond, breakingBond, angle1, imaginariesString, lithiumAlkoxide, lithiumSulfonate, carbonLithiumSulfonate, dihedral1
